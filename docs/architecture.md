...
│                                                             │
│  ┌─────────────────┐ ┌──────────────────┐ ┌──────────────┐  │
│  │   HTTP Router   │ │  Session Mapper  │ │ SSE Producer │  │
│  │   (main.ts)     │ │  (sessions.ts)   │ │  (sse.ts)    │  │
│  └────────┬────────┘ └────────┬─────────┘ └──────▲───────┘  │
│           │                   │                  │          │
│  ┌────────▼────────┐ ┌────────▼─────────┐ ┌──────┴───────┐  │
│  │   RPC Manager   │ │ JSONL Fast-Path  │ │ Event Bridge │  │
│  │    (rpc.ts)     │ │  (messages.ts)   │ │ (prompt.ts)  │  │
│  └────────┬────────┘ └────────┬─────────┘ └──────────────┘  │
└───────────┼───────────────────┼─────────────────────────────┘
...
### 2. Session Management & Fast Path (`src/sessions.ts`, `src/messages.ts`)
...
### 3. OMP RPC Process Manager (`src/rpc.ts`)
...
### 4. Event Translation & SSE Stream (`src/prompt.ts`, `src/sse.ts`)
...
### 5. Approvals & Custom Extensions (`src/approvals.ts`, `extensions/`)
...
### 6. Title Generation (`src/title.ts`)
